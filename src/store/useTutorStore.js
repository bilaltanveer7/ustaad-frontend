...
export const useTutorStore = create((set, get) => ({
  ...
  fetchTutors: async (page = 1, limit = 20) => {
    set({ isLoading: true, error: null });
    
    try {
      const response = await getAllTutors(page, limit);
  ...
  fetchTutorDetails: async (tutorId) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const errorMessage = response.data?.message || 'Failed to fetch tutor details';
        set({ error: errorMessage, isLoading: false });
        return { success: false, error: errorMessage };
      }
    } catch (error) {
  ...
  updateTutor: (tutorId, partial) =>
    set((state) => ({
      tutors: state.tutors.map((t) => (t.id === tutorId ? { ...t, ...partial } : t)),
    })),
  removeTutor: (tutorId) =>
  ...
  resetFilters: () => set({ filters: { search: '', subject: 'all', status: 'all' } }),
